# lambda_s3_ec2_metrics.tf

# --- Lambda 함수 정의 ---
data "archive_file" "lambda_s3_ec2_metrics_zip" {
  type        = "zip"
  output_path = "${path.module}/lambda_s3_ec2_metrics.zip" # 임시 zip 파일 경로
...
# 선택: OpenSearch 인덱스 접두사 (기본값: ec2-metrics)
index_prefix = os.environ.get('INDEX_PREFIX', 'ec2-metrics')
        ...
        try:
            ...
            for line in lines:
                ...
                try:
                    ...
                    # --- 벌크 API 라인 생성 ---
                    # 인덱스 이름 생성 (예: ec2-metrics-2025-04-28)
...
resource "aws_lambda_function" "s3_to_ec2_metrics_lambda" {
  filename         = data.archive_file.lambda_s3_ec2_metrics_zip.output_path
  source_code_hash = data.archive_file.lambda_s3_ec2_metrics_zip.output_base64sha256
  function_name = "s3-to-opensearch-ec2_metrics"
...
resource "aws_lambda_permission" "allow_s3_invocation_ec2_metrics" {
  statement_id  = "AllowS3InvokeFunction"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.s3_to_ec2_metrics_lambda.function_name
  principal     = "s3.amazonaws.com"
  source_arn    = aws_s3_bucket.ec2_metrics_bucket.arn
  source_account = data.aws_caller_identity.current.account_id
}
resource "aws_s3_bucket_notification" "ec2_metrics_bucket_notification" {
  bucket = aws_s3_bucket.ec2_metrics_bucket.id
  lambda_function {
    lambda_function_arn = aws_lambda_function.s3_to_ec2_metrics_lambda.arn
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "ec2-metrics/"
  }
  depends_on = [aws_lambda_permission.allow_s3_invocation_ec2_metrics]
}
